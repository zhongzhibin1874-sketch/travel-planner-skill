...
本 Skill 仅提供旅行规划建议、公开信息核实、费用估算和可用的预订入口：

- 不连接或调度支付系统，也不会代替用户付款。
- 不会真正预订酒店、机票、火车票、门票或餐厅。
- 不会代替用户提交护照、身份证、签证申请或其他敏感资料。
- 价格、库存、班次、开放时间和政策可能随时变化，最终以下单页面及官方信息为准。
- 所有购买、预订、取消和资料提交操作都必须由用户本人确认并完成。
...
- Rough per-person total trip cost estimates during destination selection
...
This skill only provides travel-planning guidance, public-information verification, cost estimates, and available booking links:

- It does not connect to or operate payment systems and cannot pay on the user's behalf.
- It does not actually book hotels, flights, trains, attraction tickets, or restaurants.
- It does not submit passports, identity documents, visa applications, or other sensitive information.
- Prices, availability, schedules, opening hours, and policies may change; booking pages and official sources are authoritative.
- Every purchase, reservation, cancellation, and information submission must be reviewed and completed by the user.
